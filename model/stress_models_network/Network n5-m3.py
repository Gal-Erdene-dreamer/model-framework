"""
Model file created on 2021-08-20 13:49:31.054210 
...
variables = ['ABC', 'AE', 'AD', 'BCE', 'B', 'ACE', 'AB', 'AC', 'ABE', 'ADE', 'BC', 'BCD', 'CE', 'BDE', 'A', 'ACD', 'CD', 'DE', 'E', 'ABD', 'CDE', 'D', 'BE', 'Kd', 'C', 'BD', 'A_tot', 'B_tot', 'E_tot', 'D_tot', 'C_tot']
...
dependent_species = ['AB', 'AC', 'AD', 'AE', 'BC', 'BD', 'BE', 'CD', 'CE', 'DE', 'ABC', 'ABD', 'ABE', 'ACD', 'ACE', 'ADE', 'BCD', 'BCE', 'BDE', 'CDE']
# The constant terms in the model
constants = ['Kd']
# Which components are labeled / detected in the experiment
labeled_components = ['ABC', 'AE', 'AD', 'ACE', 'AB', 'AC', 'ABE', 'ADE', 'A', 'ACD', 'ABD'] 
...
def system_equations(concentrations, state):
    ...
    result = np.zeros(5)
    result[0] = (A*B*C/Kd**2 + A*B*D/Kd**2 + A*B*E/Kd**2 + A*B/Kd + A*C*D/Kd**2 + A*C*E/Kd**2 + A*C/Kd + A*D*E/Kd**2 + A*D/Kd + A*E/Kd + A - A_tot) / A_tot
    result[1] = (A*B*C/Kd**2 + A*B*D/Kd**2 + A*B*E/Kd**2 + A*B/Kd + B*C*D/Kd**2 + B*C*E/Kd**2 + B*C/Kd + B*D*E/Kd**2 + B*D/Kd + B*E/Kd + B - B_tot) / B_tot
    result[2] = (A*B*E/Kd**2 + A*C*E/Kd**2 + A*D*E/Kd**2 + A*E/Kd + B*C*E/Kd**2 + B*D*E/Kd**2 + B*E/Kd + C*D*E/Kd**2 + C*E/Kd + D*E/Kd + E - E_tot) / E_tot
    result[3] = (A*B*D/Kd**2 + A*C*D/Kd**2 + A*D*E/Kd**2 + A*D/Kd + B*C*D/Kd**2 + B*D*E/Kd**2 + B*D/Kd + C*D*E/Kd**2 + C*D/Kd + D*E/Kd + D - D_tot) / D_tot
    result[4] = (A*B*C/Kd**2 + A*C*D/Kd**2 + A*C*E/Kd**2 + A*C/Kd + B*C*D/Kd**2 + B*C*E/Kd**2 + B*C/Kd + C*D*E/Kd**2 + C*D/Kd + C*E/Kd + C - C_tot) / C_tot
    return result 
...
def data_function(state):
    ...
    return A*data_min/A_tot + data_max*(AB + ABC + ABD + ABE + AC + ACD + ACE + AD + ADE + AE)/A_tot
