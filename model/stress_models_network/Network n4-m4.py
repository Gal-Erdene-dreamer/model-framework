"""
Model file created on 2021-08-20 13:48:48.805166 
...
variables = ['ABC', 'A', 'AD', 'ACD', 'BCD', 'B', 'CD', 'ABD', 'AB', 'AC', 'D', 'BC', 'ABCD', 'Kd', 'C', 'BD', 'A_tot', 'B_tot', 'D_tot', 'C_tot']
...
dependent_species = ['AB', 'AC', 'AD', 'BC', 'BD', 'CD', 'ABC', 'ABD', 'ACD', 'BCD', 'ABCD']
# The constant terms in the model
constants = ['Kd']
# Which components are labeled / detected in the experiment
labeled_components = ['ABC', 'A', 'AD', 'ACD', 'ABD', 'AB', 'AC', 'ABCD'] 
...
def system_equations(concentrations, state):
    ...
    result = np.zeros(4)
    result[0] = (A*B*C*D/Kd**3 + A*B*C/Kd**2 + A*B*D/Kd**2 + A*B/Kd + A*C*D/Kd**2 + A*C/Kd + A*D/Kd + A - A_tot) / A_tot
    result[1] = (A*B*C*D/Kd**3 + A*B*C/Kd**2 + A*B*D/Kd**2 + A*B/Kd + B*C*D/Kd**2 + B*C/Kd + B*D/Kd + B - B_tot) / B_tot
    result[2] = (A*B*C*D/Kd**3 + A*B*D/Kd**2 + A*C*D/Kd**2 + A*D/Kd + B*C*D/Kd**2 + B*D/Kd + C*D/Kd + D - D_tot) / D_tot
    result[3] = (A*B*C*D/Kd**3 + A*B*C/Kd**2 + A*C*D/Kd**2 + A*C/Kd + B*C*D/Kd**2 + B*C/Kd + C*D/Kd + C - C_tot) / C_tot
    return result 
...
def data_function(state):
    ...
    return A*data_min/A_tot + data_max*(AB + ABC + ABCD + ABD + AC + ACD + AD)/A_tot
